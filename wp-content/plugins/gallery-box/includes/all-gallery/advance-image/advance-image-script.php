...
function gbox_advance_img_gallery_script($id){
...
(function ($) {
    ...
    <?php
        ...
        do_action( 'gbox_advance_masonry_loadmore_active', $id );
...
add_action('gbox_advance_img_script', 'gbox_advance_img_gallery_script');


// Masonary active script
function gbox_advance_masonry_loadmore_active_script($id){
...
$gbox_uniqe_loadmore =  !empty( $settings_main[0]['uniqe_loadmore'])  ? $settings_main[0]['uniqe_loadmore'] : 'default';
$img_main = get_post_meta($id, 'img_main', true);
// count for Load more button 
  $total_imgaes_cunt = count($img_main);
...
add_action('gbox_advance_masonry_loadmore_active','gbox_advance_masonry_loadmore_active_script');